 ...
 *
 * */
/*此处使用farkas lemma 对系数进行了约束,然后直接使用了fm消去进行了消去\lambda 
得到了有关于c系数的约束范围,此时在这个范围内的所有系数取值都是合法的*/
...
PlutoConstraints *farkas_lemma_affine(const PlutoConstraints *dom, const PlutoMatrix *phi)
{
    ...
    printf("farkas matrix size:%d,%d",idom->ncols+idom->nrows,phi->ncols+idom->nrows);
