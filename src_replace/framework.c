...
static void compute_permutability_constraints_dep(Dep *dep, PlutoProg *prog)
{
    ...
    //print_constraints_to_file(dpoly, "output.txt", "dpoly");
    //print_matrix_to_file(phi, "output.txt", "phi_1 before Farkas");
    /* Apply Farkas lemma for tiling validity constraints */
    tiling_valid_cst = farkas_lemma_affine(dpoly, phi);
    print_constraints_to_file(tiling_valid_cst, "valid_constraints.txt", "valid_constraints");
    ...
    //print_matrix_to_file(phi, "output.txt", "phi_2 before Farkas");
    //print_constraints_to_file(dep->bounding_poly, "output.txt", "dep->bounding_poly");    
    /* Apply Farkas lemma for bounding function constraints */
    bounding_func_cst = farkas_lemma_affine(dep->bounding_poly, phi);
    //print_constraints_to_file(bounding_func_cst, "output.txt", "bounding_func_cst after Farkas");
...
//     // Print dpoly information to file
//     print_constraints_to_file(dpoly, "output.txt", "dpoly");
...
//     // Print phi information to file before applying Farkas lemma
//     print_matrix_to_file(phi, "output.txt", "phi before Farkas");

//     // Apply Farkas lemma for tiling validity constraints
//     tiling_valid_cst = farkas_lemma_affine(dpoly, phi);

//     // Print tiling_valid_cst information to file after applying Farkas lemma
//     print_constraints_to_file(tiling_valid_cst, "output.txt", "tiling_valid_cst after Farkas");
...
//     // Print bounding_func_cst information to file after applying Farkas lemma
//     print_constraints_to_file(bounding_func_cst, "output.txt", "bounding_func_cst after Farkas");

//     pluto_matrix_free(phi);
//     pluto_constraints_free(dpoly);
//     printf("this function is over");
...
PlutoConstraints *get_permutability_constraints(PlutoProg *prog)
{
    ...
    nstmts = prog->nstmts;
    ndeps = prog->ndeps;
    printf("依赖维度%d in get_permutability_constraints\n",ndeps);
    deps = prog->deps;
    nvar = prog->nvar;
    npar = prog->npar;
    ...
    for (i=0; i<ndeps; i++) {
        Dep *dep = deps[i];
    // 对不同的依赖种类都会去算一下,例如存在rar,war,raw,则npdes为3,循环计算三次依赖确定系数范围
