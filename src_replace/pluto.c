        ...
        int max_sols, int band_depth)
{
    ...
    // modifyied by lqz
    /*此处get_permutability_constraints中会调用farkas lemma对现有的依赖多面体以及phi进行farkas lemma,此时已经有了对
    变量约束的范围就是我们想要的空间!!!*/
    ...
    printf("%d num_sols_found in find_permutable_hyperplanes\n",num_sols_found);
...
// 函数：打印 PlutoMatrix 的 val 数组内容到文件
void printPlutoMatrixValToFile(FILE *fp, PlutoMatrix *matrix) {
    if (matrix == NULL) {
        fprintf(fp, "PlutoMatrix is NULL\n");
        return;
    }
    // 遍历并打印 val 数组的内容
    for (int i = 0; i < matrix->nrows; i++) {
        for (int j = 0; j < matrix->ncols; j++) {
            fprintf(fp, "%d ", matrix->val[i][j]);
        }
        fprintf(fp, "\n");
    }
}
...
char **merge_vars_and_params(char **vars, int nvars, char **params, int nparams) {
    // 创建一个新数组，长度为 nvars + nparams + 1
    char **result = malloc((nvars + nparams + 1) * sizeof(char *));
    if (result == NULL) {
    ...
    for (int i = 0; i < nparams; i++) {
        result[nvars + i] = strdup(params[i]);
        if (result[nvars + i] == NULL) {
            perror("Failed to duplicate string");
            // 清理已分配的内存
    ...
    // 添加常数项 "1"
    result[nvars + nparams] = strdup("1");
    if (result[nvars + nparams] == NULL) {
        perror("Failed to duplicate string");
        // 清理已分配的内存
        for (int j = 0; j < nvars + nparams; j++) {
            free(result[j]);
        }
        free(result);
        return NULL;
...
void affine_transform_print(FILE *fp, PlutoMatrix *mat, PlutoProg *prog, char **vars) {
    int ndims = prog->nvar + prog->npar + 1; // 变量+参数+常量列

    for (int i = 0; i < mat->nrows; i++) {
        int first = 0; // 标记是否为当前表达式的第一项
        for (int j = 0; j < mat->ncols - 1; j++) { // 遍历变量系数（最后一列是常量）
            long long coeff = mat->val[i][j];
            if (coeff != 0) {
                // 处理符号和系数
                if (coeff == 1) {
                    fprintf(fp, "%s%s", (first ? "+" : ""), vars[j]);
                } else if (coeff == -1) {
                    fprintf(fp, "-%s", vars[j]);
                } else {
                    // 非±1系数时，用 * 连接（如 +2*i 或 -3*j）
                    fprintf(fp, "%s%lld*%s", 
                            (coeff > 0 && first ? "+" : ""), 
                            coeff, vars[j]);
                }
        ...
        // 处理常量项
        long long constant = mat->val[i][mat->ncols - 1];
        if (constant != 0) {
            fprintf(fp, "%s%lld", (constant > 0 && first ? "+" : ""), constant);
        } else if (!first) {
            fprintf(fp, "0"); // 全零表达式
        }
        fprintf(fp, " "); // 表达式分隔符（空格）
    }
}



ScheduleSize printPlutoscheduelToFile(PlutoProg *prog,int stmt_no){
    char filename[256];
    int nstmts = prog->nstmts;
    int nvar = prog->nvar;
    int npar = prog->npar;
    ...
    int nrows =0;
    int ncols = nvar+npar+1;
    sprintf(filename, "schedule_%d.txt", stmt_no);

    FILE *schedule_file = fopen(filename, "w");
    int numscalar = 0;
    for(int I=0;I<stmts[stmt_no]->trans->nrows;I++){
            if(stmts[stmt_no]->hyp_types[I]== H_SCALAR){
                numscalar++;   
            }
            else{
                for(int K=0;K<nvar+npar+1;K++){
                    
                    printf("%d ",stmts[stmt_no]->trans->val[I][K]);
                    fprintf(schedule_file,"%d ",stmts[stmt_no]->trans->val[I][K]);
    ...
    printf("printPlutoscheduelToFile is ok\n");
    printf(" %d,%d \n",nrows,ncols);
    scheduleSize.nrows = nrows;
    scheduleSize.ncols = ncols;
    return scheduleSize;
}

PlutoMatrix* createPlutoMatrix (const char* filename,int nrows ,int ncols ,PlutoProg *prog){
    printf(" %d,%d \n",nrows,ncols);
    PlutoMatrix* matrix = (PlutoMatrix *)malloc(sizeof(PlutoMatrix));
    matrix->nrows = nrows;
    matrix->ncols = ncols;
    matrix->alloc_nrows = nrows;
    matrix->alloc_ncols = ncols;
    matrix->val = (int64 **)malloc(nrows * sizeof(int64*));
    
    for (int i = 0; i < matrix->nrows; i++) {
        matrix->val[i] = (int64 *)malloc(matrix->ncols * sizeof(int64));
    }
    ...
    for (int i = 0; i < matrix->nrows; i++) {
        for (int j = 0; j < matrix->ncols; j++) {
            fscanf(file, "%lld ", &matrix->val[i][j]);
            printf("%lld ",matrix->val[i][j]);
        }
        printf("\n");
    }
    fclose(file);
    printf("create schedule is ok\n");
    return matrix;   
}
...
void writePolyhedralModelToFile(const char *filename, PlutoProg *prog) {
    FILE *info_fp = fopen(filename, "w");
    int schedule_cols;
    if (!info_fp) {
        fprintf(stderr, "Error opening file to write  polyhedral model\n");
        exit(1);
    }
    /* 遍历所有语句 */
    for (int i = 0; i < prog->nstmts; i++) {
        ...
        char **vars = merge_vars_and_params(stmt->iterators,prog->nvar,prog->params,prog->npar);
        
        fprintf(info_fp, "Statement %d Iterators:\n", stmt->id+1);
        for (int iter = 0; iter < stmt->dim; iter++) {
            fprintf(info_fp, "%s\n", stmt->iterators[iter]);
        }
        fprintf(info_fp, "//\n");
        /* 写入语句文本 */
        fprintf(info_fp, "Statement %d: %s\n", i+1, stmt->text);
        fprintf(info_fp, "//\n");
        /*写入语句的结构参数信息*/
        fprintf(info_fp, "Par %d:\n",i+1);
        for (int i = 0; i < prog->npar; i++) {
            fprintf(info_fp, "%s\n", prog->params[i]);
        }
        fprintf(info_fp, "//\n");
        /* 写入语句的域信息 */
        fprintf(info_fp, "Domain %d: ",i+1);
        pluto_constraints_pretty_print(info_fp, stmt->domain);
        fprintf(info_fp, "//\n");
        /* 写入读取访问信息 */
        fprintf(info_fp, "Reads %d:\n",i+1);
        for (int j = 0; j < stmt->nreads; j++) {
            PlutoAccess *access = stmt->reads[j];
            fprintf(info_fp, "%s:", access->name);
            affine_transform_print(info_fp,access->mat,prog,vars);
            fprintf(info_fp, "\n");
        }
        fprintf(info_fp, "//\n");
        /* 写入写入访问信息 */
        fprintf(info_fp, "Writes %d:\n",i+1);
        for (int j = 0; j < stmt->nwrites; j++) {
            PlutoAccess *access = stmt->writes[j];
            fprintf(info_fp, "%s:", access->name);
            affine_transform_print(info_fp,access->mat,prog,vars); 
            fprintf(info_fp, "\n");
        }
        fprintf(info_fp, "//\n");
        fprintf(info_fp, "Schedule %d:\n",i+1);
        ScheduleSize scheduleSize = printPlutoscheduelToFile(prog, i);
        char schedulefile[256];
        sprintf(schedulefile, "schedule_%d.txt", i);
        nrows = scheduleSize.nrows;
        ncols = scheduleSize.ncols;
        PlutoMatrix *matrix = createPlutoMatrix(schedulefile,nrows,ncols,prog);
        for (int i = 0; i < matrix->nrows; i++)
        {
            for (int j = 0; j < matrix->ncols; j++)
            {
        ...
        affine_transform_print(info_fp,matrix,prog,vars);
        fprintf(info_fp, "\n");
        fprintf(info_fp, "//\n");
        fprintf(info_fp, "------------------------\n");
...
static void print_constraints_to_file(PlutoConstraints *constraints, const char *filename, const char *label) {
    FILE *file = fopen(filename, "a");
    if (file != NULL) {
        fprintf(file, "%s:\n", label);
        for (int i = 0; i < constraints->nrows; i++) {
            for (int j = 0; j < constraints->ncols; j++) {
                fprintf(file, "%d ", constraints->val[i][j]);
            }
            fprintf(file, "\n");
        }
        fprintf(file,"111111");
...
static void print_matrix_to_file(PlutoMatrix *matrix, const char *filename, const char *label) {
    FILE *file = fopen(filename, "a");
    if (file != NULL) {
        fprintf(file, "%s:\n", label);
        for (int i = 0; i < matrix->nrows; i++) {
            for (int j = 0; j < matrix->ncols; j++) {
                fprintf(file, "%d ", matrix->val[i][j]);
            }
            fprintf(file, "\n");
        }
        fclose(file);
...
// PlutoMatrix *read_schedule(const char *inputFileName, int maxrows, int maxcols) {
//     // Allocate memory for PlutoMatrix
//     PlutoMatrix *matrix = (PlutoMatrix *)malloc(sizeof(PlutoMatrix));
//     if (matrix == NULL) {
//         printf("Failed to allocate memory for PlutoMatrix.\n");
//         return NULL;
//     }

//     // Initialize matrix properties
//     matrix->nrows = maxrows;
//     matrix->ncols = maxcols;
//     matrix->alloc_nrows = maxrows;
//     matrix->alloc_ncols = maxcols;

//     // Allocate memory for the 2D array (val field)
//     matrix->val = (int64_t **)malloc(maxrows * sizeof(int64_t *));
//     if (matrix->val == NULL) {
...
//         matrix->val[i] = (int64_t *)malloc(maxcols * sizeof(int64_t));
//         if (matrix->val[i] == NULL) {
//             printf("Failed to allocate memory for matrix columns.\n");
//             for (int k = 0; k < i; k++) {
//                 free(matrix->val[k]);
//             }
//             free(matrix->val);
...
//             fscanf(inputFile, "%ld", &matrix->val[i][j]);
//         }
//     }
...
//     return matrix;
// }
PlutoMatrix *read_schedule(const char *inputFileName) {
    // Open the input file
    FILE *inputFile = fopen(inputFileName, "r");
    if (inputFile == NULL) {
        printf("Failed to open file %s.\n", inputFileName);
    ...
    for (int i = 0; i < maxrows; i++) {
        for (int j = 0; j < maxcols; j++) {
            if (fscanf(inputFile, "%ld", &matrix->val[i][j]) != 1) {
                printf("Failed to read matrix value.\n");
...
void replace_trans(Stmt **stmts,int nstmts,int nvar,PlutoMatrix *mmaxtrix){
    int **maxtrix = mmaxtrix->val;
    int maxrow = mmaxtrix->nrows;
    printf("replace_trans star work\n");
                    ...
                    for(int I=0;I<stmts[J]->trans->nrows;I++){
                        if(stmts[J]->hyp_types[I]== H_SCALAR){
                            ...
                            for(int K=0;K<nvar;K++){
                                stmts[J]->trans->val[I][K] = maxtrix[I-numscalar][J*(nvar+1)+K];
                            }
                        }
                    }
                }
